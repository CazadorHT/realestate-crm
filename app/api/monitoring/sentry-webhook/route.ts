...
export async function POST(req: NextRequest) {
  ...
  if (secret !== process.env.MONITORING_SECRET) {
    logger.warn("Unauthorized monitoring access attempt", { source: "monitoring-webhook" });
  ...
  try {
    ...
    if (Array.isArray(tags)) {
      tags.forEach((t: any) => { tagMap[t.key || t[0]] = t.value || t[1]; });
    }

    const title = body.message || issue.title || "Unknown Error";
    const environment = event.environment || tagMap["environment"] || process.env.NODE_ENV || "unknown";
    ...
    if (title.toLowerCase().includes("finance") || sourceFile.toLowerCase().includes("finance")) {
      icon = "💸";
      priority = "CRITICAL";
    } else if (title.toLowerCase().includes("ai") || title.toLowerCase().includes("gemini") || sourceFile.toLowerCase().includes("inngest")) {
      icon = "🤖";
      priority = "HIGH";
...
<i>Sent via VC Connect Command Center</i>
    ...
    // 5. Zero Latency Dispatch: Send without awaiting (Serverless friendly logging)
    sendAdminNotification(telegramMessage, { parseMode: "HTML" }).catch(err => {
      logger.error("Failed to dispatch telegram alert", err, { source: "monitoring-webhook" });
    });

    return NextResponse.json({ success: true });

  } catch (error) {
    logger.error("Sentry Webhook Payload Error", error, { source: "monitoring-webhook" });
