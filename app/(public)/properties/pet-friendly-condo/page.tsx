...
export default async function PetFriendlyCondoPage(props: {
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-orange-50/20 pt-(--nav-offset,0px) transition-[padding-top] duration-300 ease-in-out">
      {/* WebPage Schema */}
      <script
                ...
                <div className="flex flex-wrap items-center gap-2 pt-4 text-xs font-bold text-slate-500 animate-fade-in-up">
                  ...
                  {popularAreas.map((area) => {
                    ...
                    return (
                      <Link 
                        key={area.name}
                        href={`/properties/pet-friendly-condo?popular_area=${encodeURIComponent(area.name)}`}
                        className="px-3 py-1.5 rounded-full bg-white border border-slate-100 text-slate-600 hover:bg-orange-600 hover:text-white transition-all shadow-3xs flex items-center gap-1.5"
                      >
                        <span>{localizedLabel}</span>
                        <span className="text-[10px] opacity-60 text-orange-600">({area.count})</span>
                      </Link>
                    );
                  })}
                </div>
