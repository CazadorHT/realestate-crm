    ...
    <div className="space-y-6 max-w-full mx-auto">
      ...
      <Breadcrumb
        ...
        items={[
          ...
          { label: isEn ? "Edit Owner" : "แก้ไขข้อมูล" },
        ]}
      />
      <div className="flex items-center gap-4 px-6 py-8 border-b border-slate-200 bg-linear-to-r from-slate-800 to-slate-900 rounded-xl">
        ...
        <div className="space-y-1">
          <h1 className="text-2xl font-bold text-white">
            {isEn ? "Edit Property Owner" : "แก้ไขข้อมูลเจ้าของทรัพย์"}
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-200 flex items-center gap-3 bg-linear-to-r from-slate-800 to-slate-900">
            ...
            <div>
              <h2 className="font-semibold text-white">{isEn ? "Owner Information" : "ข้อมูลเจ้าของ"}</h2>
              <p className="text-sm text-slate-300">{isEn ? "Edit contact details" : "แก้ไขข้อมูลติดต่อ"}</p>
            </div>
          </div>
          <div className="p-6">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-6 py-4  bg-linear-to-r from-slate-800 to-slate-900 border-b border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="font-semibold text-white">{isEn ? "Properties" : "รายการทรัพย์"}</h2>
                <p className="text-sm text-slate-300">
                  {isEn ? `Total ${properties.length} listings` : `ทั้งหมด ${properties.length} รายการ`}
                </p>
              </div>
            </div>
            <Button asChild size="sm" variant="add" className="gap-2 cursor-pointer">
          ...
          {properties.length > 0 ? (
            <div className="divide-y divide-slate-100">
              {properties.map((prop) => {
                const typeLabel =
                  (PROPERTY_TYPE_LABELS as any)[prop.property_type]?.[lang] ||
            ...
            <div className="p-12 text-center">
              ...
              <h3 className="font-medium text-slate-900 mb-1">
                {isEn ? "No properties yet" : "ยังไม่มีทรัพย์"}
              </h3>
              <p className="text-sm text-slate-500 mb-4">
                {isEn ? "This owner has no properties linked yet." : "เจ้าของนี้ยังไม่มีทรัพย์ในระบบ"}
              </p>
              <Button asChild size="sm" variant="outline" className="cursor-pointer">
                <Link href={`/protected/properties/new?owner_id=${owner.id}`}>
                  <Plus className="h-4 w-4 mr-2" />
                  {isEn ? "Add First Property" : "เพิ่มทรัพย์แรก"}
                </Link>
              </Button>
            </div>
          )}
