          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-2xl md:text-3xl font-bold text-white">
                {isEn ? "Trash" : "ถังขยะ (Trash)"}
              </h1>
            </div>
            <p className="text-red-100 text-sm md:text-base max-w-md ml-11">
              {isEn ? (
                <>
                  Manage deleted properties, restore, or permanently remove • Total{" "}
                  <span className="font-bold text-white">{count}</span> items
                </>
              ) : (
                <>
      ...
      <div className="bg-red-50 border border-red-200 text-red-800 rounded-lg p-4 flex gap-3 items-start">
        ...
        <div className="space-y-1">
          ...
          <p className="text-sm opacity-90">
            {isEn
              ? "Items in the trash will be retained for 30 days before being permanently deleted from the system."
              : "รายการในถังขยะจะถูกเก็บไว้เป็นเวลา 30 วันก่อนที่จะถูกลบออกจากระบบอย่างถาวร"}
          </p>
        </div>
      </div>
    </>
