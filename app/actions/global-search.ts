...
export async function globalSearchAction(query: string): Promise<SearchResult[]> {
  ...
  const [propertiesRes, leadsRes, dealsRes, agentsRes, ownersRes] = await Promise.all([
    ...
    (async () => {
      ...
      if (tokens.some(t => t.includes("โฮมออฟฟิศ") || t.includes("home office") || t.includes("homeoffice"))) smartFilters.push(`property_type.eq.HOME_OFFICE`);
      else if (tokens.some(t => t.includes("คอนโด"))) smartFilters.push(`property_type.eq.CONDO`);
      else if (tokens.some(t => t.includes("บ้าน") || t.includes("เดี่ยว"))) smartFilters.push(`property_type.eq.HOUSE`);
      else if (tokens.some(t => t.includes("ทาวน์"))) smartFilters.push(`property_type.eq.TOWNHOME`);
      else if (tokens.some(t => t.includes("พูลวิลล่า"))) smartFilters.push(`property_type.eq.POOL_VILLA`);
      ...
      if (tokens.some(t => (t.includes("ออฟฟิศ") || t.includes("สำนักงาน") || t.includes("office")) && !t.includes("โฮมออฟฟิศ") && !t.includes("home office"))) smartFilters.push(`property_type.eq.OFFICE_BUILDING`);
