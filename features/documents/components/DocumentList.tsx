...
import { DocumentOwnerType, DocumentType, DOC_TYPE_LABELS, DOC_TYPE_LABELS_EN } from "../schema";
import {
  getDocumentsByOwner,
...
export function DocumentList({
  ...
  const handleView = async (storagePath: string) => {
    ...
    if (url) {
      ...
      toast.error(isEn ? "Failed to open file" : "ไม่สามารถเปิดไฟล์ได้");
    }
  };
  ...
  const handleDelete = async (id: string, storagePath: string) => {
    ...
    if (res.success) {
      toast.success(isEn ? "File deleted successfully" : "ลบไฟล์สำเร็จ");
      fetchDocs();
    } else {
      toast.error(isEn ? "Failed to delete file" : "ลบไฟล์ไม่สำเร็จ");
      throw new Error(isEn ? "Failed to delete file" : "ลบไฟล์ไม่สำเร็จ");
    }
  };
  ...
  if (documents.length === 0) {
    return (
      <div className="text-center py-6 text-muted-foreground border border-dashed rounded-md text-xs">
        {isEn ? "No documents found" : "ยังไม่มีเอกสาร"}
      </div>
    );
  }
  ...
  return (
    <div className="grid grid-cols-1 gap-4 scrollbar-thin scrollbar-thumb-slate-200 scrollbar-track-transparent hover:scrollbar-thumb-slate-300 transition-colors">
      {documents.map((doc) => {
        ...
        const docTypeLabel = isEn
          ? (DOC_TYPE_LABELS_EN[docTypeUpper] || doc.document_type || "Other")
          : (DOC_TYPE_LABELS[docTypeUpper] || doc.document_type || "อื่นๆ");
              ...
              <div className="flex-1 p-3 flex items-center justify-between gap-2 md:gap-4 min-w-0">
                ...
                <div className="shrink-0 flex items-center gap-1">
                  <ResponsiveDialog
                    ...
                    description={isEn ? "Select an action for this document" : "เลือกคำสั่งสำหรับเอกสารนี้"}
                  ...
                  >
                    <div className="p-4 space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <DocumentPreviewDialog
                          ...
                          trigger={
                            <Button
                              variant="ghost"
                              className="w-full justify-start h-12 rounded-xl px-4 gap-3 font-bold text-slate-700 hover:bg-blue-50 hover:text-blue-600 border border-transparent hover:border-blue-100 cursor-pointer"
                            >
                              <div className="h-8 w-8 rounded-lg bg-blue-100/50 flex items-center justify-center">
                                <Eye className="h-4 w-4" />
                              </div>
                              {isEn ? "Preview Document" : "ดูตัวอย่างเอกสาร"}
                            </Button>
                          }
                        />
                        ...
                        <VersionHistoryDialog
                          ...
                          trigger={
                            <Button
                              variant="ghost"
                              className="w-full justify-start h-12 rounded-xl px-4 gap-3 font-bold text-slate-700 hover:bg-slate-50 hover:text-slate-900 border border-transparent hover:border-slate-100 cursor-pointer"
                            >
                        ...
                        {ownerType === "LEAD" && (
                          <ESignDialog
                            ...
                            trigger={
                              <Button
                                variant="ghost"
                                className="w-full justify-start h-12 rounded-xl px-4 gap-3 font-bold text-slate-700 hover:bg-amber-50 hover:text-amber-600 border border-transparent hover:border-amber-100 cursor-pointer"
                              >
                                ...
                                {isEn ? "E-Signature" : "ส่งเซ็น E-Signature"}
                              </Button>
                            }
                          />
                        )}
                        ...
                        <AIDocumentInsight
                          ...
                          trigger={
                            <Button
                              variant="ghost"
                              className="w-full justify-start h-12 rounded-xl px-4 gap-3 font-bold text-slate-700 hover:bg-purple-50 hover:text-purple-600 border border-transparent hover:border-purple-100 cursor-pointer"
                            >
                              <div className="h-8 w-8 rounded-lg bg-purple-100/50 flex items-center justify-center">
                                <div className="w-4 h-4 bg-purple-400 rounded-full animate-pulse" />
                              </div>
                              {isEn ? "AI Analysis" : "วิเคราะห์ด้วย AI"}
                            </Button>
                          }
                        />
                      </div>
                      ...
                      <ConfirmDialog
                        ...
                        confirmText={isEn ? "Delete Permanently" : "ลบออกถาวร"}
                        variant="destructive"
                        onConfirm={() => handleDelete(doc.id, doc.storage_path)}
                        trigger={
                          <Button
                            variant="ghost"
                            className="w-full justify-start h-12 rounded-xl px-4 gap-3 font-bold text-rose-600 hover:bg-rose-50 hover:text-rose-700 border border-transparent hover:border-rose-100 group cursor-pointer"
                          >
                            <div className="h-8 w-8 rounded-lg bg-rose-100/50 flex items-center justify-center group-hover:bg-rose-100">
                              <Trash className="h-4 w-4" />
                            </div>
                            {isEn ? "Delete This Document" : "ลบเอกสารนี้"}
                          </Button>
                        }
                      />
                    </div>
