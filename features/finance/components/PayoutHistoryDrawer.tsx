    ...
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent className="sm:max-w-md border-l-slate-100 overflow-y-auto">
        <SheetHeader className="pb-6 border-b border-slate-50">
          ...
          <SheetTitle className="text-2xl font-semibold text-slate-900">{isEn ? "Audit History" : "บันทึกการตรวจสอบ"}</SheetTitle>
          <SheetDescription className="text-slate-500">
            {isEn ? "Complete audit trail and status change history" : "Audit Trail และประวัติการเปลี่ยนแปลงสถานะทั้งหมด"}
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="py-8 space-y-8">
          ...
          <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100 space-y-1">
            <p className="text-[10px] uppercase font-bold text-slate-400 tracking-widest">{isEn ? "Transaction ID" : "ID รายการ"}</p>
            ...
            <div className="pt-2">
              <Badge variant="outline" className="bg-white border-slate-200 text-slate-600 text-[10px] font-bold">
                {selectedPayout.property?.title || (isEn ? "Unknown Property" : "ไม่ทราบชื่อทรัพย์สิน")}
              </Badge>
            </div>
          </div>
          ...
          <div className="space-y-6 relative ml-4">
            ...
            {(selectedPayout.audit_meta || [
              { action: isEn ? 'Commission record detected' : 'ระบบตรวจพบบันทึกคอมมิชชัน', timestamp: selectedPayout.created_at, author: 'System' },
              { action: isEn ? 'Commission payout approved' : 'อนุมัติการจ่ายคอมมิชชัน', timestamp: selectedPayout.updated_at, author: selectedPayout.author?.name || 'Administrator' }
          ...
          <div className="flex items-center gap-3 p-4 bg-emerald-50/50 rounded-2xl border border-emerald-100/50">
            ...
            <p className="text-[11px] text-emerald-700 font-medium">
              {isEn 
                ? "This record is secured via an immutable audit trail and cannot be altered." 
                : "บันทึกนี้ถูกบันทึกผ่านระบบ Blockchain-style Audit Trail ไม่สามารถย้อนกลับหรือแก้ไขได้"}
            </p>
          </div>
        </div>
      </SheetContent>
