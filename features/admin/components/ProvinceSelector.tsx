...
export function ProvinceSelector({
  ...
  const filteredProvinces = useMemo(() => {
    return provinces.filter((p) =>
      ...
      ((p as any).name_en && (p as any).name_en.toLowerCase().includes(search.toLowerCase()))
    );
  }, [provinces, search]);
  ...
  return (
    <>
      <Button
        ...
        disabled={loading}
        className={cn(
          "w-full justify-between h-11 bg-slate-50 border-slate-200 hover:bg-white hover:border-indigo-200 focus:ring-2 focus:ring-indigo-500/10 transition-all rounded-xl px-4 text-left font-medium cursor-pointer",
        ...
        <div className="flex flex-col h-full bg-white">
          <div className="p-4 border-b border-slate-50 sticky top-0 bg-white/80 backdrop-blur-sm z-10">
            <div className="relative">
              ...
              <Input
                placeholder={isEn ? "Search province..." : "ค้นหาจังหวัด..."}
                value={search}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 overflow-y-auto max-h-[30vh] p-2 space-y-1 scrollbar-thin">
            {filteredProvinces.length === 0 ? (
              <div className="py-20 text-center text-slate-400 col-span-full">
                <p className="text-sm">{isEn ? "No province found" : "ไม่พบข้อมูลจังหวัดที่ค้นหา"}</p>
              </div>
            ) : (
              filteredProvinces.map((p) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center justify-between p-3 rounded-xl text-sm font-bold transition-all group cursor-pointer",
                      isSelected
