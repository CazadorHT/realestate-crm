...
export function StationCombobox({
  ...
  // Filter stations based on selected transit type (BTS/MRT/ARL) and search query
  const filteredStations = React.useMemo(() => {
    return stations.filter((station) => {
      // 1. Filter by transit type if specified
      if (transitType && transitType !== "OTHER") {
        const stationType = station.metadata?.transit_type || "";
        // Group GOLD and BTS together, MRT startsWith MRT, etc.
        if (transitType === "BTS" && stationType !== "BTS" && stationType !== "GOLD") return false;
        if (transitType === "MRT" && !stationType.startsWith("MRT")) return false;
        if (transitType === "ARL" && stationType !== "ARL") return false;
      }
  ...
  }, [stations, transitType, searchQuery]);
