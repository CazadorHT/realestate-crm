...
export async function getAgentKpiStats(
  ...
  return agents.map((agent: any) => {
    const agentDeals = (deals || []).filter((d: any) => d.created_by === agent.id);
    ...
    const salesCount = agentDeals.filter((d: any) => d.deal_type === "SALE").length;
    const rentCount = agentDeals.filter((d: any) => d.deal_type === "RENT").length;
