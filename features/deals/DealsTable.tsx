...
export function DealsTable({
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      if (selectedPropertyId !== undefined) params.set("property_id", selectedPropertyId);
  ...
  }, [debouncedQ, page, pageSize, reloadKey, selectedPropertyId, selectedLeadId]);
  ...
  const StatusBadge = ({ status }: { status: string }) => {
    ...
    return (
      <Badge variant="outline" className={`${styleMap[status] || "bg-gray-100"}`}>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 w-72">
            ...
            <PropertyCombobox
              ...
              onChange={(id) => {
                ...
                if (id === null) {
                  ...
                  import("sonner").then(({ toast }) => toast.info("การกรองด้วย 'ไม่ผูกทรัพย์' ยังไม่รองรับ"));
                } else {
          ...
          {selectedPropertyId ? (
            <Button size="sm" variant="ghost" onClick={() => setSelectedPropertyId(undefined)} title="ล้าง">
              <X className="h-4 w-4" />
            </Button>
          ) : null}
        </div>
        ...
        <div className="flex items-center gap-2">
          <DealFormDialog
            trigger={<Button><Plus className="mr-2 h-4 w-4"/> สร้าง Deal</Button>}
            leadId={""}
        ...
        <TableBody>
          {data.map((deal) => (
            <TableRow key={deal.id}>
              <TableCell>
                <Link href={`/protected/properties/${deal.property_id}`} className="font-medium hover:underline">
                  {deal.property?.title || "-"}
                </Link>
              </TableCell>
              <TableCell>
                <Link href={`/protected/leads/${deal.lead_id}`} className="text-sm text-muted-foreground hover:underline">
                  {deal.lead?.full_name || "-"}
                </Link>
              </TableCell>
              <TableCell>
              ...
              <TableCell className="text-right">
                <div className="flex items-center justify-end gap-2">
                  <Link href={`/protected/deals/${deal.id}`} className="btn btn-ghost">
      ...
      <div className="flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          แสดง {Math.min(count, (page-1)*pageSize+1)} - {Math.min(count, page*pageSize)} จาก {count}
        </div>
        <div className="flex items-center gap-2">
          <Button disabled={page <= 1} onClick={() => setPage((p) => Math.max(1, p - 1))}>ก่อนหน้า</Button>
          <div className="px-2 py-1 rounded border bg-muted text-sm">{page} / {totalPages}</div>
          <Button disabled={page >= totalPages} onClick={() => setPage((p) => Math.min(totalPages, p + 1))}>ถัดไป</Button>
        </div>
      </div>
    </div>
  );
