...
export function DealsFilters({
  ...
  const propertyTypeLabelMap: Record<string, string> = {
    ...
    TOWNHOME: isEn ? "Townhome" : "ทาวน์โฮม",
    HOME_OFFICE: isEn ? "Home Office" : "โฮมออฟฟิศ",
        ...
        >
          <div className="p-4 grid grid-cols-1 gap-2 max-h-[60vh] overflow-y-auto no-scrollbar">
            {[
              ...
              { label: isEn ? "Townhome" : "ทาวน์โฮม", value: "TOWNHOME", icon: Layout, color: "text-amber-600", bg: "bg-amber-50", count: stats?.property_type?.TOWNHOME },
              { label: isEn ? "Home Office" : "โฮมออฟฟิศ", value: "HOME_OFFICE", icon: Building2, color: "text-indigo-600", bg: "bg-indigo-50", count: stats?.property_type?.HOME_OFFICE },
