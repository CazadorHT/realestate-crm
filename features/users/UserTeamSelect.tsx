...
export function UserTeamSelect({
  ...
  const handleTeamChange = async (newTeamId: string) => {
    ...
    try {
      ...
      if (result.success) {
        toast.success(isEn ? "Team updated successfully" : "อัปเดตทีมเรียบร้อยแล้ว");
        setOpen(false);
        router.refresh();
      } else {
      ...
      toast.error(isEn ? "Failed to update team" : "เกิดข้อผิดพลาดในการดําเนินการ");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className={cn("inline-flex items-center gap-2", className)}>
      <ResponsiveDialog
        ...
        title={isEn ? "Team Assignment" : "สังกัดทีม (Team Assignment)"}
        description={isEn ? "Assign staff to an operational team for permissions and reporting" : "เลือกทีมที่พนักงานคนนี้สังกัดอยู่ เพื่อจัดการสิทธิ์และรายงานผล"}
        trigger={
          <Button
            variant="outline"
          ...
          >
            ...
            <span className="font-semibold  text-sm truncate max-w-[100px]">
              {currentTeam?.name || (isEn ? "No Team" : "ไม่มีทีม")}
            </span>
            {isLoading ? (
        ...
        <div className="flex flex-col h-full max-h-[60vh]">
          ...
          <div className="sticky top-0 z-30 p-4 bg-white/80 backdrop-blur-xl border-b border-slate-100 italic">
            <div className="relative">
              ...
              <Input
                placeholder={isEn ? "Search team name..." : "ค้นหาชื่อทีม..."}
                value={searchQuery}
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            ...
            <TeamItem
              id="none"
              name={isEn ? "--- No Team Assigned ---" : "--- ไม่มีทีมสังกัด ---"}
            ...
            {filteredTeams.length === 0 && searchQuery && (
              <div className="py-12 flex flex-col items-center justify-center text-slate-400 gap-3">
                ...
                <p className="text-sm font-semibold italic">
                  {isEn ? "No teams found matching search" : "ไม่พบทีมที่คุณค้นหา"}
                </p>
              </div>
            )}
          </div>
