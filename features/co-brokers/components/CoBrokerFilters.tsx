  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-between gap-4">
      <div className="relative flex-1 max-w-md group">
        ...
        <Input
          placeholder="ค้นหาชื่อคู่ค้า, พื้นที่, หรือเบอร์โทร..."
      ...
      <div className="flex items-center gap-1 bg-slate-100/50 p-1.5 rounded-2xl border border-slate-200/50">
         <span className="text-[10px] font-bold text-slate-400 px-3 uppercase tracking-widest border-r border-slate-200 mr-1">เรตติ้ง</span>
         {[null, 3, 4, 5].map((val) => (
           <Button
              key={String(val)}
              variant={ratingFilter === val ? "secondary" : "ghost"}
              size="sm"
              className={`h-9 px-4 rounded-xl text-[10px] uppercase font-bold tracking-wider transition-all ${
                ratingFilter === val 
