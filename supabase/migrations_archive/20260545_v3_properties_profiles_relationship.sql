-- Description: Add social sharing columns to properties_core and update properties view bridge + define computed relationship

BEGIN;

-- 1. Add social sharing columns to public.properties_core if not exist
ALTER TABLE public.properties_core 
  ...
  ADD COLUMN IF NOT EXISTS posted_to_line_at TIMESTAMPTZ,
  ADD COLUMN IF NOT EXISTS posted_to_tiktok_at TIMESTAMPTZ;

-- 2. Drop the existing properties view CASCADE (to cleanly recreate with new columns)
DROP VIEW IF EXISTS public.properties CASCADE;

-- 3. Recreate public.properties view (God Tier Full Unroll with social columns)
...
-- 4. Recreate property_images view bridge (since dropping public.properties CASCADE might have dropped dependent views/grants)
...
-- 5. Define computed relationship to link public.properties and public.profiles views in PostgREST
