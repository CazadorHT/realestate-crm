...
DROP FUNCTION IF EXISTS public.update_property_elite(UUID, UUID, UUID, BOOLEAN, INTEGER, JSONB);
...
DECLARE
  ...
  v_set_clause TEXT := '';
  v_col_name TEXT;
  v_blacklist TEXT[] := ARRAY['id', 'tenant_id', 'version', 'created_at', 'created_by', 'owner_id', 'updated_at', 'status'];
BEGIN
  ...
  -- 4.1 Explicitly handle ENUM status casting to bypass jsonb_populate_record 42804 error
  IF p_data ? 'status' THEN
    v_set_clause := v_set_clause || format('status = %L::public.property_status, ', p_data->>'status');
  END IF;
