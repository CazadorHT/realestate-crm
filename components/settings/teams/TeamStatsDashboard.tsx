  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
      {cards.map((card, idx) => (
        ...
        >
          {/* BG Gradient Glow */}
          <div className={`absolute -right-10 -top-10 w-32 h-32 bg-linear-to-br ${card.gradient} opacity-5 blur-3xl group-hover:opacity-10 transition-opacity`} />
          ...
          <div className="flex items-start justify-between relative z-10">
            <div>
              ...
              <p className="text-[10px] font-semibold text-slate-400 mt-2 flex items-center gap-1 uppercase tracking-widest">
                <span className={`w-1.5 h-1.5 rounded-full bg-linear-to-r ${card.gradient}`} />
                {card.label}
              </p>
            </div>
            
            <div className={`p-4 rounded-2xl bg-linear-to-br ${card.gradient} shadow-lg shadow-indigo-100 group-hover:scale-110 transition-transform duration-500`}>
