              ...
              <ConfirmDialog
                ...
                trigger={
                  <Button
                    variant="ghost"
                    type="button"
                    className="w-full sm:w-auto text-red-500 hover:text-red-650 hover:bg-red-50 font-bold gap-2 rounded-2xl h-12 px-6 transition-all"
                  >
      ...
      >
        <div className="py-4 space-y-8 pb-10">
          ...
          <div className="space-y-3">
            <Label className="text-xs font-bold text-slate-500 uppercase tracking-wider">ภาษาของชุดคำสั่ง (Language)</Label>
            <div className="grid grid-cols-4 gap-2">
              {(["th", "en", "cn", "ru"] as const).map((lang) => {
                const labelMap = { th: "ภาษาไทย", en: "English", cn: "中文", ru: "Русский" };
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <Label className="text-xs font-bold text-slate-500 uppercase tracking-wider">จำกัดโพสต์ (Post Filter)</Label>
              </div>
              <Button
                type="button"
                variant="outline"
              ...
              >
                ...
                {item.linked_post_id ? "เปลี่ยนโพสต์" : "เลือกโพสต์"}
              </Button>
            </div>
            ...
            {!item.linked_post_id && (
              <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl">
                ...
                <span className="text-xs text-slate-500 font-medium">ใช้กับทุกโพสต์ (ค่าเริ่มต้น)</span>
              </div>
            )}
            ...
            {item.linked_post_id && (
              <div className="flex items-center gap-3 p-3 bg-violet-50 border border-violet-100 rounded-2xl">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-bold text-violet-700">เฉพาะโพสต์ที่เลือก ✓</p>
                  <p className="text-[11px] text-slate-500 mt-0.5 truncate">ID: {item.linked_post_id}</p>
                  <p className="text-[10px] text-slate-400 mt-0.5">Keyword จะทำงานเฉพาะ Comment ในโพสต์นี้</p>
                </div>
                <Button
                  variant="ghost"
                  size="icon"
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[13px] font-bold text-slate-700">เมื่อลูกค้าคอมเมนต์คำว่า...</span>
              </div>
              <ConfirmDialog
                title="ลบชุดคำสั่ง"
                description={`คุณแน่ใจหรือไม่ที่จะลบชุดคำสั่งสำหรับ Keyword "${item.keyword || "ว่าง"}"?`}
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[13px] font-bold text-slate-700">ให้ระบบทำงานดังนี้:</span>
            </div>
              ...
              <div className="space-y-3 pt-2">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                      ปุ่มแบนเนอร์/แชทบอท (DM Buttons)
                    </label>
                  </div>
                  {(item.buttons?.length || 0) < 3 && (
                    <Button
                      ...
                      onClick={() => {
                        ...
                        onUpdate({ buttons: [...currentButtons, { title: "ปุ่มใหม่", type: "postback" }] });
                      }}
                ...
                {item.buttons && item.buttons.length > 0 ? (
                      ...
                      <div key={idx} className="space-y-3 p-3 bg-white border border-slate-200/60 rounded-xl relative">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div className="space-y-1">
                            <Label className="text-[10px] font-bold text-slate-500">ชื่อปุ่ม (สูงสุด 20 ตัวอักษร)</Label>
                            <Input
                              placeholder="เช่น ปรึกษาแอดมิน"
                              maxLength={20}
                              value={btn.title}
                              onChange={(e) => {
                          ...
                          <div className="space-y-1">
                            <Label className="text-[10px] font-bold text-slate-500">ประเภทการทำงาน</Label>
                            <select
                            ...
                            >
                              <option value="postback">พิมพ์ข้อความตอบกลับ (Quick Reply)</option>
                              <option value="web_url">เปิดลิงก์เว็บไซต์ (Web URL)</option>
                            </select>
                          </div>
                        </div>
                        {btn.type === "web_url" && (
                          <div className="space-y-1">
                            <Label className="text-[10px] font-bold text-slate-500">ลิงก์ URL ปลายทาง</Label>
                            <Input
                  ...
                  <div className="text-center py-4 bg-slate-50/30 border border-dashed border-slate-200 rounded-2xl text-[11px] text-slate-400">
                    ยังไม่ได้เพิ่มปุ่มนำทาง (ข้อความจะส่งไปเฉพาะตัวอักษร)
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">
                      2. คอมเมนต์ตอบกลับสาธารณะ (สุ่ม Spintax)
                    </label>
                  </div>
                  <Button
                    variant="outline"
