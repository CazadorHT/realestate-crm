      ...
      {isMobile && (
        ...
        >
          <div 
            ...
            className="p-6 space-y-6 text-slate-700"
          >
            {/* Preview card inside dialog/drawer */}
            <div className="flex gap-4 p-3 bg-slate-50 rounded-2xl border border-slate-100 items-center">
              {displayImages.length > 0 && (
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider block">
                {language === "th" ? "ลิงก์อสังหาริมทรัพย์" : "Property Link"}
              </label>
              <div className="flex gap-2">
                <input
                  type="text"
                  readOnly
                  value={`${window.location.origin}/properties/${property.slug || property.id}`}
                  className="flex-1 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-xs font-medium text-slate-600 focus:outline-hidden"
                />
                <button
                  onClick={(e) => {
                    e.preventDefault();
                  ...
                  className="bg-slate-900 hover:bg-slate-800 text-white rounded-xl px-4 py-2 text-xs font-bold transition-colors flex items-center gap-1.5 shrink-0"
                >
                  {copied ? (
                    <>
            ...
            <div className="grid grid-cols-2 gap-3">
              {canShare && (
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-3 text-xs font-bold rounded-xl bg-slate-100 hover:bg-slate-200 transition-colors text-slate-800"
                >
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 px-4 py-3 text-xs font-bold rounded-xl bg-[#06C755] hover:bg-[#05b34d] text-white transition-colors"
              >
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 px-4 py-3 text-xs font-bold rounded-xl bg-[#1877F2] hover:bg-[#115fc4] text-white transition-colors"
                style={{ gridColumn: canShare ? "span 1" : "span 2" }}
              >
                <svg className="w-4 h-4 fill-current" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
