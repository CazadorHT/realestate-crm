...
import { ChevronDown, SlidersHorizontal, Building, Building2, Check, Loader2 } from "lucide-react";
...
export function ProjectPropertiesClient({ initialProperties, initialTotal, project }: ProjectPropertiesClientProps) {
  const { language, t: globalT } = useLanguage();
  const [properties, setProperties] = useState<PublicPropertyNearStation[]>(initialProperties || []);
  const [totalCount, setTotalCount] = useState<number>(
    initialTotal !== undefined ? initialTotal : project.propertyCount || (initialProperties || []).length
  );
  ...
  const fetchFilteredProperties = useCallback(async (newFilter: FilterType, newPropType: string) => {
    ...
    try {
      ...
      const res = await fetch(`/api/public/projects/${project.id}/properties?${params.toString()}`);
      if (res.ok) {
  ...
  const sortedProperties = useMemo(() => {
    ...
    result.sort((a, b) => {
      ...
      // default: newest (featured first, then created_at)
            ...
            {hasMore && (
              <div className="text-center pt-4">
                <button
                  ...
                  className="px-8 py-3.5 bg-white hover:bg-slate-50 border border-slate-200/80 hover:border-slate-300 text-slate-700 hover:text-slate-900 rounded-2xl text-sm font-bold cursor-pointer transition-all duration-200 shadow-xs hover:shadow-md inline-flex items-center gap-2.5 active:scale-95 disabled:opacity-60"
                >
                  {isLoadingMore ? (
                    <>
