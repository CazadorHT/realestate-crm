  ...
  const handleBulkUpdate = (status: PropertyStatus) => {
    ...
    startTransition(async () => {
      try {
        ...
        const msg = error instanceof Error ? error.message : "เกิดข้อผิดพลาดในการอัปเดต";
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          ...
          size="sm"
          disabled={isPending}
          className="h-11 text-xs bg-white hover:bg-slate-50! border-slate-200! text-slate-700! font-bold shadow-sm"
        >
          {isPending ? (
      ...
      <DropdownMenuContent align="end" className="w-56 p-2 rounded-xl shadow-xl border-slate-100">
        <DropdownMenuLabel className="text-[10px] font-bold text-slate-400 uppercase tracking-widest px-2 py-1.5">
          เลือกสถานะใหม่ ({selectedIds.length} รายการ)
        </DropdownMenuLabel>
        <DropdownMenuSeparator className="my-1 bg-slate-50" />
        
        {/* Active Group */}
        <div className="px-2 py-1.5 text-[10px] font-bold text-blue-600/60 uppercase">Market States</div>
        ...
        {/* Closed Group */}
        <div className="px-2 py-1.5 text-[10px] font-bold text-emerald-600/60 uppercase">Final States</div>
        {["SOLD", "RENTED"].map((s) => (
          <DropdownMenuItem
            key={s}
            onClick={() => handleBulkUpdate(s as PropertyStatus)}
        ...
        {/* Other Group */}
        <div className="px-2 py-1.5 text-[10px] font-bold text-slate-600/60 uppercase">Management</div>
