...
function StatusIndicator({ ok, warn }: { ok: boolean; warn?: boolean }) {
  ...
  const textColor = ok ? "text-emerald-700" : warn ? "text-amber-700" : "text-rose-700";
  const bgColor = ok ? "bg-emerald-50" : warn ? "bg-amber-50" : "bg-rose-50";

  return (
    <div className={cn("flex items-center gap-2 px-2.5 py-1 rounded-full border shadow-sm", bgColor, ok ? "border-emerald-100" : warn ? "border-amber-100" : "border-rose-100")}>
    ...
    <div className={cn(
      ...
      <div className="flex items-center gap-3">
        <div className={cn(
          ...
          ok ? "bg-emerald-50 border-emerald-100 text-emerald-600" : "bg-rose-50 border-rose-100 text-rose-500"
        )}>
          <Icon size={18} />
        </div>
        <div className="flex flex-col">
          <h4 className="text-sm font-semibold text-slate-800 tracking-tight">{name}</h4>
  ...
  if (loading) {
    return (
      <div className="p-8 flex flex-col items-center justify-center bg-slate-50/50 rounded-3xl border border-dashed border-slate-200">
        ...
        <p className="text-xs font-medium text-slate-400">กำลังตรวจสอบระบบ...</p>
      </div>
    );
  }
      ...
      >
        <div className="flex items-center gap-3.5">
          <div className={cn(
            ...
            isExpanded ? "bg-blue-600 shadow-blue-200" : "bg-white border border-slate-200 shadow-none"
          )}>
            <HiOutlineBolt size={22} className={isExpanded ? "text-white" : "text-blue-600"} />
          </div>
          <div>
            <h3 className="text-lg font-semibold text-slate-900 tracking-tight flex items-center gap-2">
              ...
              {!isExpanded && (
                <span className={cn(
                  ...
                  healthPercentage > 80 ? "bg-emerald-100 text-emerald-600" : "bg-amber-100 text-amber-600"
                )}>
                  {healthPercentage}% Healthy
                </span>
              )}
            </h3>
            <p className="text-xs text-slate-500 font-medium mt-0.5">ตรวจสอบการเชื่อมต่อและโครงสร้างพื้นฐาน</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-4 bg-white px-5 py-2.5 rounded-2xl border border-slate-200 shadow-sm">
            <div className="flex flex-col items-end">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-none mb-1">ความพร้อม</span>
              <span className={cn(
            ...
            <div className="flex -space-x-2">
              {[status.supabase?.configured, status.line?.configured, status.ai?.configured].map((ok, i) => (
                <div key={i} className="h-8 w-8 rounded-full border-2 border-white bg-slate-50 flex items-center justify-center overflow-hidden relative shadow-sm">
                  <div className={cn("absolute inset-0 opacity-15", ok ? "bg-emerald-500" : "bg-slate-400")} />
                  <HiOutlineCheckBadge size={14} className={ok ? "text-emerald-600 z-10" : "text-slate-400 z-10"} />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-2 hover:bg-slate-200 rounded-full transition-colors text-slate-400">
            {isExpanded ? <ChevronDown className="h-5 w-5 rotate-180 transition-transform" /> : <ChevronRight className="h-5 w-5 transition-transform" />}
          </div>
        </div>
      </div>
        ...
        <div className="animate-in fade-in slide-in-from-top-2 duration-500 space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 px-1">
                ...
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">โครงสร้างพื้นฐานหลัก</span>
              </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 px-1">
                <HiOutlineChatBubbleLeftRight size={16} className="text-slate-400" />
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">ข้อความและโซเชียล</span>
              </div>
              <div className="flex flex-col gap-3">
                <IntegrationCard
                  icon={SiLine}
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 px-1">
                ...
                <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">เอไอและระบบอัตโนมัติ</span>
              </div>
              <div className="flex flex-col gap-3">
                <IntegrationCard
                  icon={SiGoogle}
          ...
          {healthPercentage < 100 && (
            <div className="flex items-start sm:items-center gap-3 p-4 bg-amber-50/80 border border-amber-200/60 rounded-2xl mt-2">
              <HiOutlineExclamationTriangle size={20} className="text-amber-500 shrink-0 mt-0.5 sm:mt-0" />
